...
def validatePassport(passport):
    ...
    if re.search("#[0-9a-f]{6}", passport["hcl"]) is None or len(passport["hcl"]) != 7: return False
    if passport["ecl"] not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]: return False
    if re.search("[0-9]{9}", passport["pid"]) is None or len(passport["pid"]) != 9: return False
